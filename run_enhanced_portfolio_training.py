...
def plot_enhanced_performance(results, output_dir, tickers, enhancement_config):
    ...
    if 'final_portfolio_values' in results:
        # Considera se è una deque limitata o una lista completa
        if isinstance(results['final_portfolio_values'], deque):
            portfolio_values = list(results['final_portfolio_values'])
        else:
            portfolio_values = results['final_portfolio_values']
        safe_plot(ax2, portfolio_values, 'Final Portfolio Value', 'Episodes', 'Value ($)')
    else:
        ax2.text(0.5, 0.5, "No portfolio value data available", ha='center', va='center')
        ax2.set_title('Final Portfolio Value')
    
    # Plot Sharpe ratios
    ax3 = plt.subplot(3, 2, 3)
    if 'final_sharpe_ratios' in results:
        if isinstance(results['final_sharpe_ratios'], deque):
            sharpe_ratios = list(results['final_sharpe_ratios'])
        else:
            sharpe_ratios = results['final_sharpe_ratios']
        safe_plot(ax3, sharpe_ratios, 'Sharpe Ratio', 'Episodes', 'Sharpe Ratio')
    else:
