...
# Aggiungi questo alla classe PortfolioEnvironment:
    ...
    def step(self, actions):
        ...
        for i, ticker in enumerate(self.tickers):
            ...
            prev_price_real = self.denormalize_price(ticker, prev_price)
            curr_price_real = self.denormalize_price(ticker, curr_price)

            if prev_price_real != 0:
                ret = (curr_price_real / prev_price_real) - 1
                self.returns_history[ticker].append(ret)
                pnl_per_asset[i] = prev_positions[i] * ret * prev_price_real
