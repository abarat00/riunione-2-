...
class PortfolioEnvironment:
    ...
    def calculate_optimal_trading_size(self, target_position, current_position, price, ticker, liquidity_factor=0.1):
        """
        Calculate the optimal trading size considering transaction costs and market impact.
        """
        # Denormalizza il prezzo
        real_price = self.denormalize_price(ticker, price)
        ...
        market_impact = liquidity_factor * abs(position_diff) * real_price
        commission = max(self.min_commission, self.commission_rate * abs(position_diff) * real_price)
        
        # If cost is too high relative to expected benefit, reduce trade size
        if market_impact + commission > abs(position_diff) * real_price * 0.01:  # 1% threshold
...
# Aggiungi questo alla classe PortfolioEnvironment:
    ...
    def get_portfolio_value(self):
        ...
        positions_value = 0
        for i, ticker in enumerate(self.tickers):
            denormalized_price = self.denormalize_price(ticker, self.prices[i])
            positions_value += self.positions[i] * denormalized_price
    ...
    def calculate_trading_costs(self, actions, current_prices):
        ...
        for i, action in enumerate(actions):
            if abs(action) > 1e-6:  # Se c'è un'operazione effettiva
                ...
                # Denormalizza il prezzo
                real_price = self.denormalize_price(ticker, current_prices[i])
                
                # Calcola il valore dell'ordine
                order_value = abs(action) * real_price
    ...
    def step(self, actions):
        ...
        # Salva le posizioni e i prezzi precedenti
        prev_positions = self.positions.copy()
        prev_prices = self.prices.copy()

        # Denormalizza i prezzi se inferiori a 1 (assumendo siano normalizzati)
        for i, ticker in enumerate(self.tickers):
            if prev_prices[i] < 1:
                prev_prices[i] = self.denormalize_price(ticker, prev_prices[i], "adjClose")
            if self.prices[i] < 1:
                self.prices[i] = self.denormalize_price(ticker, self.prices[i], "adjClose")

        self.position_history.append(prev_positions.copy())
        self.action_history.append(actions.copy())

        # Calcola le nuove posizioni (con clipping se abilitato)
        ...
        # 1. Componente di performance
        performance_component = percent_return * self.reward_return_weight

        # 2. Penalità per costi di trading
        if portfolio_value_prev > 0:
            trading_cost_penalty = min(0.00005, trading_costs / portfolio_value_prev * 0.01)
        else:
            trading_cost_penalty = 0.00001
        trading_component = -trading_cost_penalty

        # 3. Bonus per diversificazione
        ...
        if total_weight > 0:
            ...
            diversification_component = (diversification_score * 0.6 + uniformity_score * 0.4) * self.diversification_bonus_factor * 3.0

        # 4. Penalità per inattività
        inactivity_component = -self.inactivity_penalty if self.inactivity_penalty > 0 and np.sum(np.abs(actions)) < 0.01 else 0.0

        # 5. Bonus per trend following (se abilitato)
        trend_component = self.calculate_trend_following_bonus() if self.trend_following_factor > 0 else 0.0

        # 6. Penalità per posizioni dominanti ed esposizione eccessiva
        dominant_penalty = 0.0
        max_position = np.max(np.abs(self.positions))
        if max_position > 0.5:
            dominant_penalty = (max_position - 0.5) * 0.01

        excess_exposure = max(np.sum(np.abs(self.positions)) - 1.0, 0.0)
        exposure_penalty = excess_exposure * 0.0001

        # 7. Bonus dinamico rispetto a un benchmark mobile dei rendimenti
        window = 10
        if len(self.portfolio_values_history) >= window:
            recent_returns = np.diff(self.portfolio_values_history[-window:]) / np.array(self.portfolio_values_history[-window:-1])
            benchmark_return = np.mean(recent_returns)
        else:
            benchmark_return = 0.0
        dynamic_bonus = 0.0
        if percent_return > benchmark_return:
            dynamic_bonus = 0.01 * (percent_return - benchmark_return)

        # 8. Bonus per eventi di calendario (se disponibili)
        ...
        if self.calendar is not None:
            ...
            if current_date is not None:
                ...
                if high_importance_events:
                    if np.sum(np.abs(actions)) < 0.05:
                        calendar_component += 0.0005 * len(high_importance_events)
                    total_exposure = np.sum(np.abs(self.positions))
                    if total_exposure < 0.5 * self.max_portfolio_pos:
                        calendar_component += 0.0003 * len(high_importance_events)

        # 9. Bonus/malus in base alle dimensioni delle azioni
        action_magnitudes = np.abs(actions)
        small_action_penalty = -0.05 if np.max(action_magnitudes) < 0.01 else 0.0
        significant_action_bonus = 0.02 * np.sum(action_magnitudes > 0.01) if np.sum(action_magnitudes > 0.01) > 0 else 0.0

        # Calcola il reward complessivo sommando le componenti
        reward = (performance_component + trading_component + diversification_component +
                inactivity_component + trend_component - dominant_penalty -
                exposure_penalty + dynamic_bonus + calendar_component +
                small_action_penalty + significant_action_bonus)
